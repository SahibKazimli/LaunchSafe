...
def build_ai_scan_file_system(focus: str) -> str:
    ...
    return (
        ...
        "- For each finding, set `location` to `path:line` where that line is the one that "
        "proves the issue you describe in `title`/`description` (the wrong route `def` / `@`, "
        "the unauthenticated handler, the dangerous `return`, etc.) — the same code path, "
        "not a different function in the file and not a closing `)` only.\n"
        "- If the same issue appears in more than one place in the file, add optional "
        "`highlight_line_ranges` as a JSON array of inclusive `[startLine, endLine]` pairs; "
        "otherwise omit it.\n"
...
AI_AUDIT_AUTH_FLOW_SYSTEM = (
    ...
    "Rules:\n"
    "- Cite specific file:line in `location`; the line must be substantive (see "
    "the route/handler/branch that is wrong), not a lone `)` or `}` line.\n"
