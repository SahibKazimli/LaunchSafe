...
def build_ai_scan_file_system(focus: str) -> str:
    ...
    return (
        ...
        "- For each finding, set `location` to `path:line` where `line` is the first "
        "substantive line of the problem in this file (e.g. the wrong `def`, `if`, or call) — "
        "not a line that is only a closing `)`, `}`, or `]`, and not a blank line.\n"
